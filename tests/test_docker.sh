...
docker-compose -f dev.yml run django mypy my_awesome_project

# Run black with --check option
docker-compose -f dev.yml run django black --check --diff  --exclude 'migrations' ./
...
docker-compose -f dev.yml run django python manage.py makemessages
